public class Ahorcado {
    ...
    public static char[] aMinusculas(char[] col) {
        ...
        char[] ccol = new char[col.length];
        for (int i = 0; i < col.length; i++) {
            ccol[i] = aMinusculas(col[i]);
    ...
    /*
     * marcarAciertos(Collection of characters col, Collection of booleans aciertos,
     * character c)
    ...
    public static boolean[] marcarAciertos(char[] col, boolean[] aciertos, char s) {
        boolean[] caciertos = new boolean[aciertos.length];

        for (int i = 0; i < col.length; i++) {
            if (col[i] == s) {
                caciertos[i] = true;
            } else {
                caciertos[i] = aciertos[i];
            }
        }
    ...
    /*
     * palabraEnmascarada(Collection of characters col, Collection of booleans
     * aciertos) -> String
     * PRE: Length(col) = Length(aciertos)
     * POST: Returns a string composed of the characters contained in col,
     * where, if the character is a space ' ', it adds a space to the resulting
     * string,
     * for the rest of the characters, it should be noted that if the position in
     * the aciertos collection is true,
     * it adds the character from col to the string, but if it is false, it adds an
     * asterisk.
     * USE CASES:
    ...
    /*
     * Function that updates the current word state and asks the user for a new
     * letter
    ...
    public static void main(String[] args) {
        String[] palabras = { "Helicóptero", "viceconsulado", "vernáculo", "verrugoso", "Iguana", "edificante",
                "edicto", "autor", "alcalde", "PERÍMETRO" };
     ...
     * accents or diacritical marks. If the character is not a vowel or does not
     * have accents or diacritical marks,
